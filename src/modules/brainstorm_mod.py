...
async def init_brainstorm_llm(api_key: str = None):
    """Инициализация LLM для брейншторма"""
    global brainstorm_llm
    try:
        # Если API ключ не передан, попытаемся получить его из конфигурации
        if api_key is None:
            from src.core.config import config
            api_key = config.deepseek_api_key
        
        if not api_key:
            logger.warning("⚠️ API ключ DeepSeek не найден, брейншторм будет недоступен")
            brainstorm_llm = None
            return
