...
"""
                ...
                response_text += f"📚 **Современная RAG - АКТИВНАЯ** (ошибка статистики: {e})\n\n"
...
async def cmd_test_shorten(message: Message):
    ...
    for i, original in enumerate(test_names, 1):
        ...
        response_text += f"{i}. \n"
        response_text += f"📄 Было: {original[:60]}{'...' if len(original) > 60 else ''}\n"
...
async def cmd_lists_stats(message: Message):
    ...
    try:
        ...
        response_text += f"🔍 OCR доступен: {'✅' if stats.get('ocr_available', False) else '❌'}\n"
        
        if stats.get('last_update'):
            response_text += f"📅 Последнее обновление: {stats['last_update']}\n"
        
        response_text += f"\n💡 Команды:\n"
        response_text += f"• /test_shorten - тест сокращения названий\n"
        response_text += f"• /test_lists_search - тест логики поиска\n"
        response_text += f"• Используйте кнопку '📋 Проверить списки' в меню"
...
async def cmd_test_lists_search(message: Message):
    ...
    try:
        ...
        for query in test_queries:
            ...
            found, match_info = parser._search_in_text(query_lower, query_parts, test_text.lower())
